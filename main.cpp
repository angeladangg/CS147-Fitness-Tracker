...
#define BUTTON_PIN 17        // Pin for button
...
const char* password = "appleman";


const char* AWS_IP = "3.91.132.124"; // Server IP
...
void setup() {
  ...
  // Initialize button
  pinMode(BUTTON_PIN, INPUT_PULLUP); // Assuming the button is wired to GND
  digitalWrite(BUTTON_PIN, LOW); // Ensure button is not activated initially
...
void loop() {
  ...
  if (buttonState == LOW && prevButtonState == HIGH) {// if (low == pressed) and in time
    // lastButtonPress = millis();
    //prevButtonState = buttonState;

    intensityMeasuring = !intensityMeasuring; // Toggle intensity measuring

    if (intensityMeasuring) {
      Serial.println("Workout intensity measuring started.");
    } else {
      Serial.println("Workout intensity measuring stopped.");
    }
    // delay(500);
  }
   prevButtonState = buttonState;
